...
const ECG_MAP = [
  ...
  { name: 'Help-seeking as a mechanic', src: 'ECG practice, MOE admissions',
    where: 'The moves are the other half of the deck: chances happen to a student, moves are the student acting. Ask for help, book the ECG counsellor, ask the Year Head about moving up, tell parents the plan, shadow a job, build an EAE folder, ask about DSA-JC and its binding offer, treat the attachment as real, take the exchange, ask a working adult for an hour. Every move is a real scheme or a real conversation, each sets a flag, and flagged chance cards are preferred by the draw, so the game makes the true thing visible: asking opens doors. EAE and DSA cards are checked against moe.gov.sg.' },
