// PLAY (Journey v4 engine, v3 staging). Live one life forward, then again.
//
// The engine and the beat logic port from v2 whole: they are swept and they
// are right. What changed is the staging, which the v2 audit called 'a form,
// not a game': the sidebar rail is now the Pocket (one sheet), the hand of
// asks lives beside the choices instead of below twenty blocks, results day
// opens as an envelope held in the hand, a door opening is a full-screen
// beat, and the ending leads with the map instead of burying it.
//
...
import { decorate, bindGlossary } from '../ui/glossary.js?v=3.0.0';
import { openSheet, onSheetAction, setSheetFoot, close as closeSheet } from '../ui/sheet.js?v=3.0.0';
import { cue } from '../sound.js?v=3.0.0';
import { drawStoryMap, saveStoryCard } from '../ui/storymap.js?v=3.0.0';
import { skyline, envelopeArt } from '../ui/scenes.js?v=3.0.0';
import {
...
let lastYear = null;
let compareSel = [];
let ceremonyDue = false;   // a door just opened; the full-screen beat plays first
let envDoneStep = null;    // the envelope already torn for this fork step
let envOpen = false;
let rerender = () => {};
let DATA = null;
...
export function renderJourney(host, data, ctx, repaint) {
  ...
  onAction(host, {
    pocket: (btn) => {
      if (!run) return;
      const jc2 = data.copy.journey;
      openSheet(`<h2 id="sheet-title">${esc(jc2.pocketHead || 'What I carry')}</h2>${rail(data, null, null)}`, btn);
      onSheetAction({
        restart: (b) => {
          if (!restartArmed) { restartArmed = true; b.textContent = jc2.startOverArm; return; }
          closeSheet();
          setLiveRun(null);
          resetJourney();
          rerender();
...
export function resetJourney() {
  ...
  ceremonyDue = false; envDoneStep = null; envOpen = false;
  lastScreen = null;
...
function shell(inner) {
  return `
    <div class="wrap-narrow">
      <div class="j-scene arrive">${inner}</div>
    </div>`;
}
...
function metaStrip(data) {
  ...
  return `
    <div class="linestrip">
      ${strip}
      <span class="ls-meta caps">${esc(fill(jc.chapterOf, { n, total }))} ${label} ${seedChip} ${want}</span>
      <button class="pocketbtn press" type="button" data-action="pocket">${icon('d_portfolio')}<span>${esc(jc.pocketHead || 'What I carry')}</span></button>
    </div>`;
}

/** Chapter meta: the spine plus the year's own headline. */
function eraOf(age) {
  return age <= 16 ? 'school' : age <= 21 ? 'postsec' : age <= 29 ? 'twenties' : age <= 39 ? 'thirties' : 'later';
}
...
function chapterMeta(data, stage, age) {
  ...
  return `
    ${metaStrip(data)}
    <div class="scene" aria-hidden="true">${skyline(eraOf(age))}</div>
...
function turnScreen(host, stage, data, age) {
  ...
  host.innerHTML = shell(`
    ...
    ${handOpen ? handStrip(data, stage, age) : ''}
    ...
    <div class="hand">
      ...
      ${held.length ? `<div class="handcol">
        ${held.map((m) => {
          ...
          return `
          <button class="handcard ${on ? 'on' : ''}" type="button" data-action="ask" data-id="${esc(m.id)}"
...
function resolveLived(data) {
  ...
  ceremonyDue = !!justLived.doorOpened;
  pendingLived = null;
...
function forkScreen(host, stage, data, age) {
  const jc = data.copy.journey;
  // Results day arrives as an object in the hand, not a list of buttons.
  // Beat one: the envelope. Beat two: it tears, and the hope written on
  // results eve is printed on the slip, so the game proves it was listening.
  // Beat three: the hall, one line fanning into the platforms.
  if (stage.id === 's_results' && envDoneStep !== run.stepIndex) {
    host.innerHTML = shell(`
      ${chapterMeta(data, stage, age)}
      <p class="lede j-sit">${situation(data, stage)}</p>
      <div class="envwrap">
        ${envelopeArt(envOpen)}
        ${envOpen && run.hope ? `<p class="env-hope serif arrive">${esc(fill(jc.hopeLine, { hope: run.hope }))}</p>` : ''}
        <div class="btn-row" style="margin-top:var(--s-4)">
          ${envOpen
    ? `<button class="btn big press" type="button" data-action="platform">${esc(jc.platformGo || 'Step onto the platform')}</button>`
    : `<button class="btn big press" type="button" data-action="tear">${esc(jc.envelopeOpen || 'Open it')}</button>`}
        </div>
      </div>`);
    settle(host, envOpen ? 'env-open' : 'env');
    onAction(host, {
      tear: () => { envOpen = true; cue('fork'); if (navigator.vibrate) { try { navigator.vibrate(10); } catch { /* fine */ } } rerender(); },
      platform: () => { envDoneStep = run.stepIndex; envOpen = false; rerender(); },
    });
    return;
  }
  const pool = visibleChoices(stage, run, MV(data), age);
...
function doorCeremony(host, data) {
  const jc = data.copy.journey;
  const d = justLived.doorOpened;
  const name = d.text.replace('Door: ', '');
  cue('door');
  host.innerHTML = `
    <div class="doorc" role="dialog" aria-label="${esc(jc.doorOpened)}">
      <div class="doorc-stage arrive">
        <p class="caps">${esc(jc.doorOpened)}</p>
        <div class="doorc-frame" aria-hidden="true">
          <span class="doorc-light"></span>
          <span class="doorc-panel left"></span>
          <span class="doorc-panel right"></span>
        </div>
        <h1 class="serif" tabindex="-1">${icon(d.doorId)} ${esc(name)}</h1>
        <p class="small">${esc(jc.doorKeeps || 'Held for good. A held door opens choices later.')}</p>
        <button class="btn big press" type="button" data-action="stepthrough">${esc(jc.doorStep || 'Step through')}</button>
      </div>
    </div>`;
  settle(host, `doorc:${run.stepIndex}`);
  onAction(host, {
    stepthrough: () => { ceremonyDue = false; rerender(); },
  });
}

function livedScreen(host, data) {
  if (ceremonyDue && justLived.doorOpened) return doorCeremony(host, data);
  const jc = data.copy.journey;
  ...
  host.innerHTML = shell(`
    <div class="chance-out lived">
      ...
      <div class="btn-row" style="margin-top:var(--s-4)">
        <button class="btn accent" type="button" data-action="jnext">${esc(jc.continue)}</button>
  ...
  bindGlossary(host);
  onAction(host, {
    jnext: () => { justLived = null; lastYear = null; rerender(); },
    undoyear: () => {
      if (!lastYear) return;
...
function endingScreen(host, data, st) {
  ...
  host.innerHTML = `
    ...
    ? `<p class="small mute">${esc(jc[`endHome${run.home[0].toUpperCase()}${run.home.slice(1)}`])}</p>` : ''}
        ...
        ${/* The map first. It is the thing a student keeps; v2 parked it
              mid-scroll below four report sections and it went home in
              nobody's camera roll. */ ''}
        ...
        <details class="endfold" style="margin-top:var(--s-5)"><summary>${esc(jc.momentsHead || 'The moments that made it')}</summary>
          <div class="panel" style="margin-top:var(--s-3)">${moments || '<p>I lived it steadily, which is a way of living it.</p>'}</div>
        </details>
        ${(() => {
        ...
        <details class="endfold"><summary>${esc(jc.recordHead || 'The record: moves, levels, combination')}</summary>
  ...
  const paintMap = () => {
    ...
    // The card draws at 1080px and a phone shows it at a third of that, so
    // the map a student is proudest of was the one thing they could not
    // read. Tap opens it full size, scrollable.
    cv.addEventListener('click', () => {
      openSheet(`
        <h2 id="sheet-title">${esc(jc.mapHead)}</h2>
        <div style="overflow-x:auto;-webkit-overflow-scrolling:touch">
          <img src="${cv.toDataURL('image/png')}" alt="" style="width:1080px;max-width:none">
        </div>`, cv);
    });
  };
  paintMap();
...
// --------------------------------------------------------------------------
// The screen contract. The router calls render; everything above is the
// same driver v2 proved, restaged.
