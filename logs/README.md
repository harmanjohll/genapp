...
A personal learning log for Beattyians. Two frameworks from the school's own slides, **GROW by reflecting** (Gift, Rise, Own, Watch) after a lesson and **ACT on Feedback** (Acknowledge, Connect, Test) when feedback comes back, with every guiding question quoted verbatim. A competency claim asks which of the 21st Century Competencies the entry shows, and the coach holds the claim against the student's own words. A coach reads every entry the moment it is logged: a built in one that works offline, and, when the class provides a coach link, Claude in the teacher's voice, returning feedback and a competency read against MOE's wording (which competencies the writing shows, quoted, with a strength) in one call.
...
| `index.html?embed=1` | Forces the embedded layout (used automatically when the page is framed by a viewer or an LMS) |
